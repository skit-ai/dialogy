...
class CreditCardNumberEntity(BaseEntity):
    entity_type: Optional[str] = attr.ib(repr=False, default="credit-card-number", kw_only=True)
    ...
    values: List[Dict[str, Any]] = attr.ib(validator=attr.validators.instance_of(list), kw_only=True)

    @classmethod
    def from_duckling(cls, d: Dict[str, Any], alternative_index: int) -> CreditCardNumberEntity:
