...
from dialogy.types.plugins import GetWorkflowUtteranceFn, UpdateWorkflowStringFn, PluginFn
from dialogy.types.utterances import Utterance


def merge_asr_output(access: GetWorkflowUtteranceFn, mutate: UpdateWorkflowStringFn) -> PluginFn:
    def inner(workflow: Workflow) -> None:
        utterances: List[Utterance] = access(workflow)

        flat_representation: List[str] = [alternative["transcript"]
